package com.qbutton.qlucene.integration

const val resourcesRoot = "src/test/resources/testfiles"
const val rootDirName = "rootdir"
const val rootDir = "$resourcesRoot/$rootDirName"
const val nestedDirName = "nesteddir"
const val nestedDir = "$rootDir/$nestedDirName"
const val nestedFileName = "simpleFile2.txt"
const val nestedFile = "$nestedDir/$nestedFileName"
const val rootDir2Name = "rootdir2"
const val rootDir2 = "$resourcesRoot/$rootDir2Name"
const val rootDir3Name = "rootdir3"
const val rootDir3 = "$resourcesRoot/$rootDir3Name"

const val tmpDir = "tmp-test"
const val tmpTestDir = "$tmpDir/test"
const val tmpTestNestedDir = "$tmpTestDir/level1"
